...
You can find some illustrative usage examples in the example folder. Note that the on-the-fly mode is experimental, there seems to be some weirdness. Please report if you run into problems.

Note that *not all commands are available via this module* (yet). I can do what I want mostly, so expanding this module further is not a good use of my time. However, if you can do some programming, it should be relatively straightforward to add new commands ([see here](./example/readme.md), contributions are welcome!), or you can ask me to implement them by opening an issue.
...
First, install this module in your Python enviroment. This module is not in the pypi repository (yet), so you can install it directly from github using `pip`, `poetry`, or whatever tool you use. The main dependencies of this module are the `pyusb` and `pillow` modules.

After that, you need to install new drivers for the DMD device. I only used this in Windows 11, so outside that you are on your own with regards to drivers. Let me know if it works with Linux (and how), and I can add the method below.
...
I forked this module from the code from [ppozzi](https://github.com/csi-dcsc/Pycrafter6500), but I added quite a few extra functions which I needed, and added a lot of documentation.
